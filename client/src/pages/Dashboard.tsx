  ...
  const filteredProjects = useMemo(() => {
    return projectsList
      .filter(project => {
        ...
        if (selectedLanguage !== 'All' && project.language !== selectedLanguage) return false
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-border pb-3">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
              {[
                { id: 'all', label: 'All Projects', count: tabCounts.all, icon: Layers },
                { id: 'starred', label: 'Starred', count: tabCounts.starred, icon: Star },
                { id: 'shared', label: 'Shared with me', count: tabCounts.shared, icon: Users },
                ...
                return (
                  ...
                  >
                    <TabIcon size={13} className={isActive ? 'text-accent' : 'text-foreground-subtle'} />
                    <span>{tab.label}</span>
                    <span
                      className={cn(
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-40 text-xs">
                  <DropdownMenuItem onClick={() => setSortOption('modified')} className="cursor-pointer">
                    Last modified
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortOption('name')} className="cursor-pointer">
                    Name (A-Z)
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setSortOption('created')} className="cursor-pointer">
                    Created date
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          ...
          {filteredProjects.length > 0 ? (
            ...
            >
              ...
              <p className="text-xs text-foreground-muted max-w-sm mt-1 mb-4 leading-relaxed">
                No projects match your current search or filters. Try adjusting your query or resetting filters.
              </p>
          ...
          {projectsList.length === 0 && (
            <div className="flex flex-col items-center justify-center p-16 text-center rounded-2xl border border-dashed border-border bg-background-elevated/20">
              ...
              <p className="text-sm text-foreground-muted max-w-md mt-1.5 mb-6">
                Get started by creating your very first collaborative cloud project with real-time sync.
              </p>
              <Button
                variant="primary"
                onClick={() => setModalOpen(true)}

...
const RoleDropdown = ({ invite, onChange }) => {
...
export const StepInvite = () => {
  const { invites, addInvite, removeInvite, updateInviteRole } =
    useOnboardingStore();
  const [inputVal, setInputVal] = useState("");
  const [copied, setCopied] = useState(false);
  const inputRef = useRef(null);

  const tryAddEmail = (raw) => {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-1.5">
        ...
        >
          {invites.map((inv) => (
            ...
            >
              ...
              <RoleDropdown
                invite={inv}
                onChange={(role) => updateInviteRole(inv.email, role)}
              />
              <button
                onClick={(e) => {
                  e.stopPropagation();
          ...
          <input
            ...
            placeholder={
              invites.length === 0
                ? "teammate@company.com, press Enter or comma…"
                : ""
            }
        ...
        <p className="text-[11px] text-foreground-subtle">
          Press Enter or comma to add. Each person gets an Editor or Viewer
          role.
        </p>
      </div>
      ...
      {invites.length > 0 && (
        <div className="rounded-lg border border-border bg-background-elevated/50 p-3">
          <p className="text-xs text-foreground-muted">
            ...
            invite{invites.length > 1 ? "s" : ""} ready to send
          </p>
        </div>
      )}
    </div>

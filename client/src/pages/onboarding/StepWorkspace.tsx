...
const WORKSPACE_TYPES: WorkspaceTypeCard[] = [
  {
    value: 'personal',
    label: 'Personal',
...
export const StepWorkspace: React.FC = () => {
  const {
    workspaceName, workspaceSlug, workspaceType, workspaceIcon,
    setWorkspace,
  } = useOnboardingStore()
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-foreground-muted">Workspaces are where you and your team collaborate</p>
      </div>
      ...
      {/* Slug */}
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-foreground-muted">Workspace URL</label>
        <div className="flex items-center gap-0 rounded-lg border border-border bg-background-elevated overflow-hidden focus-within:ring-2 focus-within:ring-accent">
          ...
          <input
            value={workspaceSlug}
            onChange={e => setWorkspace({ workspaceSlug: slugify(e.target.value) })}
      ...
      <div className="space-y-3">
        <label className="text-xs font-medium text-foreground-muted">Workspace type</label>
          ...
          {WORKSPACE_TYPES.map(t => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <div className={cn('text-sm font-semibold', active ? 'text-accent' : 'text-foreground')}>{t.label}</div>
                  <div className="mt-0.5 text-[11px] text-foreground-subtle">{t.description}</div>
                </div>
              </button>
            )
          })}
      ...
      {/* Icon picker */}
      <div className="space-y-3">
        <label className="text-xs font-medium text-foreground-muted">Workspace icon</label>

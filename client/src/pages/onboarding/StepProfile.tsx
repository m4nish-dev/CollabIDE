...
export const StepProfile: React.FC = () => {
  const { displayName, username, bio, avatarDataUrl, setProfile } = useOnboardingStore()
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => setProfile({ avatarDataUrl: e.target?.result as string })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-foreground">Let&apos;s set up your profile</h2>
        <p className="mt-1 text-sm text-foreground-muted">How should your teammates see you?</p>
      </div>
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        >
          {avatarDataUrl ? (
            <img src={avatarDataUrl} alt="avatar" className="h-full w-full rounded-full object-cover" />
          ) : (
        ...
        <span className="text-xs text-foreground-subtle">Click or drag to upload · PNG, JPG, GIF · max 2MB</span>
        <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={e => { if (e.target.files?.[0]) handleFile(e.target.files[0]) }} />
      </div>

      {/* Display name */}
      <div className="space-y-1.5">
        <label className="text-xs font-medium text-foreground-muted">Display name</label>
        <Input
          value={displayName}
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <Input
            ...
            className={cn(
              'pr-9',
              usernameState === 'taken' && 'border-danger focus-visible:ring-danger',
              usernameState === 'available' && 'border-success focus-visible:ring-success'
            )}
          />
          <span className="absolute right-3 top-1/2 -translate-y-1/2">
            {usernameState === 'checking' && <Loader2 size={14} className="animate-spin text-foreground-subtle" />}
            {usernameState === 'available' && <CheckCircle2 size={14} className="text-success" />}
            {usernameState === 'taken' && <XCircle size={14} className="text-danger" />}
          </span>
        </div>
        {username && (
          <p className={cn(
            'text-xs',
            usernameState === 'taken' ? 'text-danger' : 'text-foreground-subtle'
          )}>
            {usernameState === 'taken'
      ...
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="text-xs font-medium text-foreground-muted">Short bio <span className="text-foreground-subtle">(optional)</span></label>
          <span className={cn('text-xs', bio.length > 140 ? 'text-warning' : bio.length > 155 ? 'text-danger' : 'text-foreground-subtle')}>
            {bio.length}/160
          </span>
        </div>
        <textarea
          value={bio}
          onChange={e => setProfile({ bio: e.target.value.substring(0, 160) })}

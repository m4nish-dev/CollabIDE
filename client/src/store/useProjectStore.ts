...
const SEED_PROJECT_FILES: FileNode[] = [
  {
    id: 'root-src',
    name: 'src',
    path: 'src',
...
export interface ProjectState {
  ...
  // Panel toggles
  activeActivity: 'explorer' | 'search' | 'git' | 'debug' | 'extensions' | 'collaborators'
  isSidebarOpen: boolean
  isBottomPanelOpen: boolean
  isRightPanelOpen: boolean
  activeBottomTab: 'terminal' | 'problems' | 'output' | 'debug'
  activeRightTab: 'preview' | 'collaborators' | 'activity' | 'comments'
  ...
  setActiveFile: (idOrPath: string) => void
  openTab: (idOrPath: string) => void
  closeTab: (idOrPath: string) => void
  saveCurrentFile: () => void
  markFileSaved: (idOrPath: string) => void
  updateFileContent: (idOrPath: string, content: string) => void
  setCursorPosition: (pos: { lineNumber: number; column: number }) => void
  toggleSplitEditor: () => void
  toggleRun: () => void
  setIsShareModalOpen: (open: boolean) => void
  
  setActiveActivity: (activity: ProjectState['activeActivity']) => void
  toggleSidebar: () => void
  toggleBottomPanel: () => void
  toggleRightPanel: () => void
  setActiveBottomTab: (tab: ProjectState['activeBottomTab']) => void
  setActiveRightTab: (tab: ProjectState['activeRightTab']) => void
  
  // Terminal actions
  addTerminal: () => void
  closeTerminal: (id: string) => void
  setActiveTerminal: (id: string) => void
  runTerminalCommand: (id: string, command: string) => void
...
function findNodeByPath(nodes: FileNode[], pathOrId: string): FileNode | null {
...
function updateNodeContent(nodes: FileNode[], pathOrId: string, content: string): FileNode[] {
  return nodes.map((node) => {
    if (node.path === pathOrId || node.id === pathOrId) {
      return { ...node, content }
    }
    if (node.children) {
      return { ...node, children: updateNodeContent(node.children, pathOrId, content) }
    }
    return node
  })
}
...
function renameNodeInTree(nodes: FileNode[], targetPath: string, newName: string): FileNode[] {
  return nodes.map((node) => {
...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  currentBranch: 'main',
  branches: ['main', 'feat/auth-flow', 'fix/cursor-sync', 'refactor/monaco-theme', 'v2-preview'],
  files: SEED_PROJECT_FILES,
  activeFileId: 'src/App.tsx',
  openTabIds: ['src/App.tsx', 'src/components/Header.tsx', 'src/components/Button.tsx'],
  ...
  terminals: [
    {
      ...
      title: 'bash (dev)',
      logs: [
        { id: '1', type: 'system', text: 'CollabIDE Cloud Sandbox v2.4.0 (node v20.12.2, linux-x64)' },
        { id: '2', type: 'input', text: 'npm run dev' },
        { id: '3', type: 'output', text: '  VITE v5.4.2  ready in 248 ms' },
        { id: '4', type: 'output', text: '  ➜  Local:   http://localhost:3000/' },
    ...
    },
    {
      ...
      logs: [
        ...
        { id: '2', type: 'output', text: 'On branch main\nYour branch is up to date with \'origin/main\'.' },
        { id: '3', type: 'output', text: 'Changes not staged for commit:\n  modified:   src/components/Header.tsx' },
      ],
    },
  ],
  ...
  collaborators: [
    {
      ...
      selection: { startLineNumber: 8, startColumn: 10, endLineNumber: 8, endColumn: 28 },
      role: 'Editor',
      status: 'online',
    },
    {
      ...
      selection: { startLineNumber: 24, startColumn: 8, endLineNumber: 24, endColumn: 36 },
      role: 'Editor',
      status: 'online',
    },
    {
  ...
  problems: [
    {
      ...
      line: 24,
      column: 15,
      message: "Unused variable 'unreadAlertsCount' is implicitly typed as number.",
      severity: 'warning',
    ...
    },
    {
      ...
      line: 42,
      column: 7,
      message: "Prop 'isLoading' might cause layout jitter if height is unconstrained.",
      severity: 'info',
      source: 'react-hints',
    },
    {
      ...
      line: 9,
      column: 26,
      message: "Type 'string' is not assignable to type 'TabKey'. Did you mean 'overview'?",
      severity: 'error',
      source: 'tsc (TS2322)',
    },
  ],
  ...
  commentThreads: [
    {
      ...
      content: 'Should we add an optional iconSlot prop here so callers don\'t have to nest manually?',
      replies: [
        {
          id: 'reply-1',
          author: 'Priya Patel',
          ...
          content: 'Agreed! Left/right icon slots would make Header button implementations much cleaner.',
        },
      ],
    },
    {
      ...
      content: 'Make sure to wrap this with the LiveSyncProvider once the WebContainer handshake finishes.',
      replies: [],
    },
  ],
  ...
  setActiveFile: (idOrPath: string) => {
    const node = findNodeByPath(get().files, idOrPath)
    if (!node || node.type === 'folder') return
    const path = node.path
    set((state) => ({
      activeFileId: path,
      openTabIds: state.openTabIds.includes(path) ? state.openTabIds : [...state.openTabIds, path],
    }))
  },

  openTab: (idOrPath: string) => {
    const node = findNodeByPath(get().files, idOrPath)
    if (!node || node.type === 'folder') return
    const path = node.path
    set((state) => ({
  ...
  updateFileContent: (idOrPath: string, content: string) => {
    const node = findNodeByPath(get().files, idOrPath)
    const path = node ? node.path : idOrPath
    set((state) => {
  ...
  setCursorPosition: (pos: { lineNumber: number; column: number }) => set({ cursorPosition: pos }),
  toggleSplitEditor: () => set((state) => ({ splitEditor: !state.splitEditor })),
  ...
  addTerminal: () => {
    set((state) => {
      ...
      const newTerm: TerminalSession = {
        ...
        title: `bash (${nextNum})`,
        logs: [
          { id: '1', type: 'system', text: `CollabIDE Cloud Sandbox terminal session #${nextNum} ready.` },
        ],
      }
      return {
  ...
  closeTerminal: (id: string) => {
    set((state) => {
      ...
      return {
        terminals: next,
        activeTerminalId: state.activeTerminalId === id ? next[0].id : state.activeTerminalId,
      }
    })
  },
  ...
  runTerminalCommand: (id: string, command: string) => {
    ...
    set((state) => {
      ...
      if (cmd === 'clear') {
        return {
          terminals: state.terminals.map((t) => (t.id === id ? { ...t, logs: [] } : t)),
        }
        ...
        newLogs.push(
          { id: `${Date.now()}-1`, type: 'output', text: '  VITE v5.4.2  ready in 190 ms' },
          { id: `${Date.now()}-2`, type: 'output', text: '  ➜  Local:   http://localhost:3000/' },
          { id: `${Date.now()}-3`, type: 'output', text: '  ➜  Network: use --host to expose' },
        )
      } else if (cmd === 'npm test' || cmd === 'npm run test') {
        newLogs.push(
          { id: `${Date.now()}-1`, type: 'output', text: 'PASS src/components/Button.test.tsx (3 tests)' },
          { id: `${Date.now()}-2`, type: 'output', text: 'PASS src/components/Header.test.tsx (4 tests)' },
          { id: `${Date.now()}-3`, type: 'output', text: 'Test Suites: 2 passed, 2 total\nTests: 7 passed, 7 total' },
        )
      } else {
        newLogs.push({
          id: `${Date.now()}-out`,
      ...
      return {
        terminals: state.terminals.map((t) => (t.id === id ? { ...t, logs: newLogs } : t)),
      }
    })
  },
  ...
  renameNode: (path: string, newName: string) => {
    set((state) => {
      ...
      const nextActive = state.activeFileId === path ? newPath : state.activeFileId
      return {
        files: nextFiles,
        openTabIds: nextTabs,
        activeFileId: nextActive,
  ...
  moveNode: (sourcePath: string, targetFolderPath: string) => {
    set((state) => {

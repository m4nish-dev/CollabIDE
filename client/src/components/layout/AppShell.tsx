...
const SidebarNavItem: React.FC<NavItemProps> = ({ item, collapsed }) => {
  ...
  const linkContent = (
    ...
    >
      ...
      <item.icon
        size={16}
        className={cn('shrink-0 transition-colors', isActive ? 'text-accent' : 'text-foreground-subtle group-hover:text-foreground-muted')}
      />
      ...
      <div className="border-t border-border p-2 space-y-1">
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  <ChevronsUpDown size={13} className="text-foreground-subtle" />
                </>
              )}
            </button>
          </TooltipTrigger>
          {collapsed && <TooltipContent side="right">{mockUser.workspace}</TooltipContent>}
        </Tooltip>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <UserAvatar name={mockUser.name} image={mockUser.avatar} size="sm" presence="online" />
              {!collapsed && (
                <>
                  <div className="flex-1 text-left min-w-0">
                    <div className="text-xs font-medium text-foreground truncate">{mockUser.name}</div>
                    <div className="text-[10px] text-foreground-subtle truncate">{mockUser.email}</div>
                  </div>
          ...
          <DropdownMenuContent side="top" align="start" className="w-52 mb-1">
            <div className="px-2 py-1.5 mb-1">
              <div className="text-xs font-semibold text-foreground">{mockUser.name}</div>
              <div className="text-[11px] text-foreground-muted">{mockUser.email}</div>
            </div>
            <DropdownMenuSeparator />
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 border-b border-border bg-background-elevated/60 backdrop-blur-sm shrink-0">
      ...
      >
        ...
        <span className="flex-1 text-left text-xs text-foreground-subtle">Search or jump to…</span>
...
export const AppShell: React.FC<AppShellProps> = ({ breadcrumbs }) => {

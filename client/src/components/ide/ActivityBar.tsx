...
export const ActivityBar: React.FC = () => {
  ...
  const topItems: Array<{
    id: ProjectState['activeActivity']
    label: string
    ...
    { id: 'git', label: 'Source Control (⌃⇧G)', icon: GitBranch, badge: unstagedCount > 0 ? unstagedCount : undefined },
    { id: 'debug', label: 'Run & Debug (⌘⇧D)', icon: Bug },
    { id: 'collaborators', label: `Collaborators (${collaborators.length})`, icon: Users },
    { id: 'extensions', label: 'Extensions (Coming soon)', icon: Puzzle, disabled: true },
  ]

  const handleItemClick = (item: typeof topItems[0]) => {
    if (item.disabled) {
      toast.info('Plugin marketplace and extensions are coming soon in next milestone!')
      return
    }
  ...
  const handleCommandPalette = () => {
    toast.info('Quick Open & Command Palette (⌘P / ⌘⇧P)')
  }

  const handleSettingsClick = () => {
    toast.info('IDE Settings: JetBrains Mono, Dark Electric Violet theme active')
  }
          ...
          {topItems.map((item) => {
            ...
            return (
              <Tooltip key={item.id}>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={`relative w-12 h-12 flex items-center justify-center transition-colors group ${
                      item.disabled ? 'opacity-40 cursor-not-allowed' : 'hover:text-foreground'
                ...
                <TooltipContent side="right" className="text-xs bg-background-overlay border-border">
                  {item.label}
                </TooltipContent>
              </Tooltip>
            )

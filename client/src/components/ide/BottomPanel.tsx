...
export const BottomPanel: React.FC<BottomPanelProps> = ({
  ...
  const tabs: Array<{
    id: ProjectState['activeBottomTab']
    label: string
    badge?: string | number
    badgeColor?: string
  }> = [
    { id: 'terminal', label: 'Terminal' },
    {
      ...
      badgeColor: errorCount > 0 ? 'bg-danger text-white' : 'bg-warning text-black',
    },
        ...
        <div className="flex-1 overflow-hidden flex">
            ...
            <div className="flex-1 flex flex-col h-full bg-[#0A0A0D]">
              ...
              >
                  ...
                  <div key={log.id} className="leading-relaxed">
                    ...
                    {log.type === 'input' && (
                      <div className="flex items-center gap-1.5 text-foreground">
                        <span className="text-emerald-400 font-bold">rohit@collabide</span>
                        <span className="text-foreground-subtle">:</span>
                        <span className="text-sky-400 font-bold">~/project</span>
                ...
                <form onSubmit={handleCommandSubmit} className="flex items-center gap-1.5 pt-1">
                  <span className="text-emerald-400 font-bold shrink-0">rohit@collabide</span>
                  <span className="text-foreground-subtle shrink-0">:</span>
                  <span className="text-sky-400 font-bold shrink-0">~/project</span>
          ...
          {activeBottomTab === 'output' && (
            <div className="flex-1 overflow-y-auto p-3 text-xs text-foreground-muted font-mono space-y-1 select-text">
              <div className="text-foreground-subtle">[14:52:10] [vite] Initializing WebContainer sandbox compiler...</div>
              <div className="text-accent">[14:52:11] [vite] Connected to hot reload channel (hmr port 3000)</div>
              <div className="text-foreground-muted">[14:52:12] [vite] Transforms compiled: 28 modules in 142ms</div>
              <div className="text-emerald-400">[14:52:12] [vite] Ready for connections at http://localhost:3000/</div>
              <div className="text-foreground-subtle">[14:54:33] [collab-sync] Broadcast peer presence: 5 connected nodes</div>
            </div>
          )}
          ...
          {activeBottomTab === 'debug' && (
            <div className="flex-1 overflow-y-auto p-3 text-xs text-foreground-muted font-mono space-y-1 select-text">
              <div className="text-foreground-subtle">Welcome to Node.js v20.12.2 Debug Console.</div>
              <div className="text-foreground-subtle">Type '.help' for more information.</div>
              <div className="text-foreground-muted">&gt; process.version</div>
              <div className="text-emerald-400">'v20.12.2'</div>
              <div className="text-foreground-muted">&gt; process.env.NODE_ENV</div>

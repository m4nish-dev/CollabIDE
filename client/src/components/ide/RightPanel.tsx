...
export const RightPanel: React.FC = () => {
  ...
  const handleAddReply = (_threadId: string) => {
  ...
  const tabs: Array<{
    id: ProjectState['activeRightTab']
    label: string
    icon: React.ComponentType<{ className?: string }>
  }> = [
    { id: 'preview', label: 'Preview', icon: Laptop },
    { id: 'collaborators', label: `Peers (${collaborators.length})`, icon: Users },
                    ...
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={`p-1 rounded ${
                          deviceMode === 'desktop' ? 'text-accent bg-accent/15' : 'text-foreground-subtle hover:text-foreground'
                        }`}
                      >
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={`p-1 rounded ${
                          deviceMode === 'tablet' ? 'text-accent bg-accent/15' : 'text-foreground-subtle hover:text-foreground'
                        }`}
                      >
                        <Tablet className="h-3.5 w-3.5" />
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="bottom">Tablet view (768px)</TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        className={`p-1 rounded ${
                          deviceMode === 'mobile' ? 'text-accent bg-accent/15' : 'text-foreground-subtle hover:text-foreground'
                        }`}
                      >
                        <Smartphone className="h-3.5 w-3.5" />
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="bottom">Mobile view (375px)</TooltipContent>
                  </Tooltip>
                </div>
                ...
                >
                  ...
                  <div className="h-10 px-3 bg-background-elevated border-b border-border flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-foreground">CollabIDE Demo</span>
                    </div>
                  ...
                  <div className="flex-1 p-4 overflow-y-auto space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-foreground-muted">
                        Write code together in real-time with sub-50ms latency, multi-cursor presence, and instant cloud previews.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <div className="p-3 rounded-lg bg-background border border-border flex items-start gap-2.5">
                        ...
                        <div>
                          <h4 className="text-xs font-semibold text-foreground">Instant Sync</h4>
                          <p className="text-[11px] text-foreground-muted">CRDT-powered operational transforms.</p>
                        </div>
                      </div>
                      <div className="p-3 rounded-lg bg-background border border-border flex items-start gap-2.5">
                        <Users className="h-4 w-4 text-secondary shrink-0 mt-0.5" />
                        <div>
                          <h4 className="text-xs font-semibold text-foreground">Peer Awareness</h4>
                          <p className="text-[11px] text-foreground-muted">Live remote cursors and terminal sessions.</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-2">
                      <button
                        onClick={() => toast.success('Action executed inside sandboxed app!')}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        onClick={() => {
                          ...
                          toast.info(`Switched to ${c.name}'s active file: ${c.activeFile.split('/').pop()}`)
                        }}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-foreground">
                        ...
                        <span className="font-mono text-accent">{act.target}</span>
                      </div>
                      <div className="text-[10px] text-foreground-subtle mt-0.5">{act.timestamp}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeRightTab === 'comments' && (
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => toast.info('Click a line number in the editor to start a discussion thread')}
                ...
                {commentThreads.map((thread) => (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      />
                      <div>
                        <div className="font-semibold text-foreground text-xs">{thread.author}</div>
                        <p className="text-foreground-muted text-xs mt-0.5">{thread.content}</p>
                      </div>
                    </div>
                        ...
                        {thread.replies.map((rep) => (
                          <div key={rep.id} className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-foreground-muted text-xs mt-0.5">{rep.content}</p>
                            </div>
                          </div>
                        ))}
                      </div>

...
interface TreeItemProps {
  ...
  collaborators: ReturnType<typeof useProjectStore.getState>['collaborators']
  onSelectFile: (path: string) => void
  onNewFileAt: (folderPath: string) => void
  onNewFolderAt: (folderPath: string) => void
...
export const FileExplorer: React.FC = () => {
  ...
  const [isCreatingFile, setIsCreatingFile] = useState<string | null>(null) // parentPath or null
  ...
  const handleNewFileSubmit = (parentPath: string) => {
  ...
  const handleNewFolderSubmit = (parentPath: string) => {
    const trimmed = inlineInputName.trim()
    if (trimmed) {
      createFolder(parentPath === 'root-project' ? '' : parentPath, trimmed)
      setExpandedPaths((prev) => new Set([...prev, parentPath ? `${parentPath}/${trimmed}` : trimmed]))
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    toast.success(`Moved ${sourcePath.split('/').pop()} to ${targetPath.split('/').pop()}`)
  }
                  ...
                  {isCreatingFile === 'root-project' && (
                    <div className="pl-6 pr-2 py-1 flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') handleNewFileSubmit('root-project')
                          if (e.key === 'Escape') setIsCreatingFile(null)
                        }}
                        className="w-full text-xs bg-background border border-accent rounded px-1.5 py-0.5 text-foreground placeholder:text-foreground-subtle focus:outline-none"
                      />
                  ...
                  {isCreatingFolder === 'root-project' && (
                    <div className="pl-6 pr-2 py-1 flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') handleNewFolderSubmit('root-project')
                          if (e.key === 'Escape') setIsCreatingFolder(null)
                        }}
                        className="w-full text-xs bg-background border border-accent rounded px-1.5 py-0.5 text-foreground placeholder:text-foreground-subtle focus:outline-none"
                      />
              ...
              <div className="max-h-36 overflow-y-auto px-2 py-1 space-y-0.5 text-xs font-mono">
                {mockSymbols.map((sym, idx) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-foreground-subtle">:{sym.line}</span>
                  </div>
                ))}
              </div>
            )}

...
const getActionIcon = (action) => {
  ...
  if (
    action.includes("merged") ||
    action.includes("PR") ||
    action.includes("reviewed")
  )
    return GitPullRequest;
  if (action.includes("deployed") || action.includes("passed"))
    return CheckCircle2;
...
export const ActivitySidebar = ({ className }) => {
  ...
  return (
    <aside
      className={cn(
        "w-full xl:w-[320px] 2xl:w-[340px] shrink-0 flex flex-col gap-5",
        className,
      )}
    >
      {/* 1. Online Teammates */}
